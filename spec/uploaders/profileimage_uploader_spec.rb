...
RSpec.describe(ProfileimageUploader) do
  def fixture_file(name)
    Rails.root.join(SPEC_FILES, name).open("rb")
  end

  def attacher_for(file)
    ProfileimageUploader::Attacher.new.tap do |attacher|
  ...
  def oversized_image_fixture
    Tempfile.new(["upload", ".png"]).tap do |file|
      ...
      file.write("a" * (ProfileimageUploader::MAX_SIZE + 1))
      file.rewind
    end
  end
  ...
  it "accepts valid profile images" do
  ...
  it "rejects oversized profile images" do
  ...
  it "rejects profile images with excessive dimensions" do
