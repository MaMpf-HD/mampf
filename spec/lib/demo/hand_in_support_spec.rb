...
# Both stagers hand sheets in with `update_all`, which skips the callback that
...
RSpec.describe(Demo::HandInSupport, type: :model) do
  ...
  describe ".record_hand_in!" do
    it "stamps the hand-in that was just built" do
      user = create(:confirmed_user)
      participation = participation_for(user, :pending)

      described_class.record_hand_in!(assignment, [user], submission)
    ...
    it "leaves somebody who was marked absent or exempt alone" do
      ...
      described_class.record_hand_in!(assignment, [absent, exempt], submission)
