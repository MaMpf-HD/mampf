...
RSpec.describe(PaginatedSearcher) do
  ...
  let(:model_searcher_instance) { instance_spy(ModelSearcher, call: search_scope) }
  let(:search_scope) { instance_spy(ActiveRecord::Relation, "SearchScope") }
  let(:paginated_array) { double("PaginatedArray") }

  before do
    # Stub ModelSearcher
    allow(ModelSearcher).to receive(:new).and_return(model_searcher_instance)
    # Stub Kaminari
  ...
  describe "#call" do
    it "initializes and calls ModelSearcher" do
      search_result
      expect(ModelSearcher).to have_received(:new).with(
        model_class,
        search_params,
        filter_classes,
        user: user
