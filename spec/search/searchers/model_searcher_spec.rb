...
RSpec.describe(Search::Searchers::ModelSearcher) do
  ...
  let(:custom_sorter_class) { class_spy(Search::Sorters::BaseSorter, "CustomSorter") }
  ...
  let(:config) do
    instance_double(
      ...
      params: params,
      sorter_class: sorter_class_from_config
    )
  end
  ...
  describe ".search" do
    # This shared example now relies on a `let(:expected_sorter)` to be defined
    ...
    context "with the default sorter" do
      let(:sorter_class_from_config) { nil }
    ...
    context "with a custom sorter class" do
      let(:sorter_class_from_config) { custom_sorter_class }
