# spec/components/search_form/filters/teacher_filter_spec.rb
require "rails_helper"

RSpec.describe(SearchForm::Filters::TeacherFilter, type: :component) do
  ...
  let(:teachers) { [["Teacher A", 1], ["Teacher B", 2]] }
  ...
  before do
    ...
    allow(I18n).to receive(:t).with("basics.teachers").and_return("Teachers")
    ...
    allow(User).to receive(:select_teachers).and_return(teachers)
  ...
  describe "#initialize" do
    it "initializes as a MultiSelectField with correct, hard-coded options" do
      expect(filter.name).to eq(:teacher_ids)
      expect(filter.label).to eq("Teachers")
      expect(filter.help_text).to eq("Help text")
    end

    it "uses the collection from User.select_teachers" do
      # Accessing the filter's collection triggers the subject's initialization.
      expect(filter.collection).to eq(teachers)
      # Now that the subject has been initialized, we can verify the method was called.
      expect(User).to have_received(:select_teachers)
    end
  end
end
