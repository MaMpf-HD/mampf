...
class WatchlistsController < ApplicationController
  before_action :sanitize_params, only: %i[show update_order change_visibility]
  ...
  def create
    ...
    @medium = Medium.find_by(id: params[:watchlist][:medium_id])
  ...
  def show
    ...
    return unless !@watchlists.empty? && !@watchlist.watchlist_entries.empty?
  ...
  def update_order
    if params[:reverse]
      params[:order].reverse.each_with_index do |id, index|
  ...
  def check_ownership
    Watchlist.find_by(id: parmas[:id]).ownedBy(current_user)
  end
end
