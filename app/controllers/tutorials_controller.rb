...
class TutorialsController < ApplicationController
  before_action :set_tutorial, only: %i[edit destroy update cancel_edit
                                        bulk_download_submissions
                                        bulk_download_corrections
                                        bulk_upload
                                        export_teams]
  before_action :set_assignment, only: %i[bulk_download_submissions
                                          bulk_download_corrections´
                                          bulk_upload
                                          export_teams]
  before_action :set_lecture, only: %i[index overview]
  before_action :set_lecture_from_form, only: [:create]
  before_action :can_view_index, only: :index
  authorize_resource except: %i[index overview create validate_certificate
                                new cancel_new]
  ...
  private
    ...
    def can_view_index
      if current_user.in?(@lecture.tutors) || current_user.editor_or_teacher_in?(@lecture)
        return
      end
