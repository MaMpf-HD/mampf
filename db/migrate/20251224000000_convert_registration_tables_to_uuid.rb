...
class ConvertRegistrationTablesToUuid < ActiveRecord::Migration[8.0]
  def up
    tables = [:registration_campaigns, :registration_items, :registration_policies,
              :registration_user_registrations]
    if tables.any? { |t| ActiveRecord::Base.connection.select_value("SELECT 1 FROM #{t} LIMIT 1") }
      raise StandardError,
            "This migration requires empty registration tables. " \
            "Please clear data or migrate manually."
    end
