...
-   Key administrative actions:
    -   **"Manage Registrations"**: Links to the `RegistrationCampaign` admin UI for that lecture. Shows status (Draft, Open, Closed, Finalized).
    -   **"Manage Rosters"**: Links to the `RegisterableRosterService` UI for managing tutorial/talk rosters post-allocation.
