...
- **Uniform API:** A `Rosterable` concern provides a consistent interface (`roster_user_ids`, `replace_roster!`, etc.).
- **Single Service:** A `RosterMaintenanceService` handles atomic moves, adds, and removals with capacity checks and logging.
- **Campaign-Independent:** Actions operate directly on `Rosterable` models; no campaign context is needed for manual changes.
...
A concern that gives any `Registerable` model a uniform roster management interface.
```
...
### Usage Scenarios
- `Tutorial` and `Talk` both include `Rosterable`.
...
| `move_user!(user_id:, from:, to:, ...)` | Atomically moves a user from one `Rosterable` to another. |
| `add_user!(user_id:, to:, ...)` | Adds a user to a `Rosterable`. |
| `remove_user!(user_id:, from:, ...)` | Removes a user from a `Rosterable`. |

### Behavior Highlights
- **Transactional:** All operations, especially `move_user!`, are performed within a database transaction to ensure atomicity.
- **Capacity Enforcement:** Enforces the `capacity` of the target `Rosterable` unless an `allow_overfill: true` flag is passed.
...
- **Moving a student:** An administrator moves a student from a full tutorial to one with free space.
  ```ruby
  service = RegisterableRosterService.new(actor: current_admin)
...
The `Tutorial` model includes the `Rosterable` concern to provide a standard interface for managing its student roster via a join table.
...
#### Rosterable Implementation
The `Talk` model includes the `Rosterable` concern to provide a standard interface for managing its speakers.
...
This diagram shows the concrete database relationships for the two example `Rosterable` implementations. The `Rosterable` concern provides a uniform API over these different underlying structures.
...
sequenceDiagram
    ...
    participant RosterService as RegisterableRosterService
    participant RosterableModel as Rosterable (e.g., Tutorial)
    ...
    note right of RosterableModel: Uses private methods from Rosterable concern
    end
```
