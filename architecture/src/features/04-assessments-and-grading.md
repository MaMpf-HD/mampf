...
- **Student submits work (digital assignment):** A student uploads their homework file. The system creates a participation record with `status: :submitted` and `submitted_at: Time.current`. The `tutorial_id` is set to the student's current tutorial membership.

- **Paper assignment grading:** For assignments with `requires_submission: false`, there is no `submitted` status. When the tutor enters points for a student, a participation is created with `status: :graded`. The `submitted_at` field remains `nil` since no file was uploaded.
