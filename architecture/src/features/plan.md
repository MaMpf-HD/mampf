...
10. **Exam Assessments & Grade Schemes**
   Action: Build the `Exam` model and its integration with the assessment system. Implement `GradeScheme::Applier` service to apply grading schemes. Add support for multiple choice exams with `Assessment::McGrader` for German legal compliance (Gleitklausel). This step will use the new grading UI built in Step 8, potentially adding minor UI elements for scheme configuration and MC exam setup.
