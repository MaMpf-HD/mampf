...
2. **Foundations (Additive Schema)**  
   Action: Create the new database tables (`registration_campaigns`, `registration_items`, `user_registrations`, `registration_policies`, `assessments`, `tasks`, `task_points`, etc.) and their corresponding Rails models. This step is purely backend and involves no UI changes.
...
7. **Assessments (Formalize `Assignment` as Assessable)**  
   Action: Add the new assessment-related models (`Assessment`, `Task`, etc.). Run a background migration to create a corresponding `Assessment` record for each existing `Assignment`. This step formalizes existing assignments as items that can be graded within the new system.
...
8. **Grading Flow & Submission Fan-out**  
   Action: Introduce the backend `GradeSubmissionService`. Build new grading UIs for instructors and TAs where they can view submissions and enter points. This UI will call the new service to save points and grades to the new tables.
...
9. **Eligibility & Exam Registration**  
   Action: Implement the backend service to compute `ExamEligibilityRecords` by reading from the new assessment and point tables. On the frontend, update the student registration UI to display eligibility status and errors. Create a new admin UI to inspect eligibility records.
...
10. **Exam Assessments & Grade Schemes**  
   Action: Build the backend functionality to create assessments for exams and apply grading schemes. This step will use the new grading UI built in Step 8, potentially adding minor UI elements for scheme configuration.
...
11. **Dashboard Integration (Phase B)**  
