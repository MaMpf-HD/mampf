...
- **Factual Source:** `LecturePerformance::Record` stores materialized performance data per (lecture, user). It contains facts only, not interpretations.
...
- **Teacher Certification:** `LecturePerformance::Certification` captures teacher-declared status (`pending`, `passed`, `failed`) with audit fields and a snapshot of the rule used when certifying. Manual overrides are encoded as `source: :manual`.
...
- **Service-Based Computation:** `LecturePerformance::Service` aggregates points and achievements and upserts the factual `LecturePerformance::Record`.
- **Evaluator (Teacher Tool):** `LecturePerformance::Evaluator` is a teacher-facing tool that interprets factual records to generate bulk certification proposals and show rule change impact. It is never called during registration/finalization runtime.
...
- **After coursework completion:** A background job runs `LecturePerformance::Service.new(lecture: ...).compute_and_upsert_all_records!`. Alice's record is created with `points_total_materialized: 58`, `percentage_materialized: 58`. The record itself does not say if she passed.
...
- **Service uses rule:** The computation service loads the active rule: `rule = LecturePerformance::Rule.find_by(lecture: lecture, active: true)` and accesses `rule.required_achievements` for evaluation. Points are aggregated from all lecture assignments.
...
```admonish info "What it represents"
A service that computes a student's performance by aggregating assessment points and checking achievements. It upserts these facts into a materialized `LecturePerformance::Record`.
```
...
| `compute_and_upsert_record_for(user)` | Computes performance for a single user and upserts their `LecturePerformance::Record`. Returns the fresh record. |
...
module LecturePerformance
  class Service
    def initialize(lecture:)
      @lecture = lecture
      @rule = lecture.lecture_performance_rule
    end
    ...
    def compute_and_upsert_record_for(user)
      ...
      LecturePerformance::Record.upsert(record_data, unique_by: [:lecture_id, :user_id])
      LecturePerformance::Record.find_by(lecture_id: @lecture.id, user_id: user.id)
    end
...
## LecturePerformance::Evaluator (Service Object)
**_Teacher-Facing Proposal Generator_**

```admonish info "What it represents"
A teacher-facing tool that interprets factual `LecturePerformance::Record` entries against a `LecturePerformance::Rule` to generate bulk certification proposals. Used exclusively in the teacher UI for bulk certification workflows and rule change impact analysis. Never called during student registration or finalization runtime.
```
...
| `evaluate(record)` | Evaluates a single `LecturePerformance::Record` and returns a structured proposal. |
...
## LecturePerformance::Certification (ActiveRecord Model)
...
- **Pre-flight validation (completeness check):**
  - **Registration phase:** When campaign with registration-phase lecture_performance policy is saved, warn if certifications are missing. On campaign open, hard-fail if any certifications are missing/pending.
  - **Finalization phase:** When campaign with finalization-phase lecture_performance policy finalizes, hard-fail if any confirmed registrants have missing/pending/failed certifications. Show remediation UI.
...
module LecturePerformance
  class Certification < ApplicationRecord
    self.table_name = "lecture_performance_certifications"
    ...
    belongs_to :rule, class_name: "LecturePerformance::Rule", optional: true
...
`kind: :lecture_performance`. Policies are evaluated by phase:
`registration`, `finalization`, or `both`. Unlike other policy types (email, deadline) that gate at runtime, lecture_performance policies enforce data completeness before the phase starts, then check Certification status at runtime.
```
...
**`LecturePerformance::Certification`** (authoritative decision layer)
...
- Creates or updates the factual `LecturePerformance::Record`.
- Used for background updates and teacher dashboards, not runtime gating.

### Policy Configuration

When a teacher wants lecture performance gating, they add a `Registration::Policy`
...
The policy queries `LecturePerformance::Rule.find_by(lecture_id: 42, active: true)` to get the actual criteria for UI display, but enforcement is done via Certification table lookups.

#### Pre-flight Validation (Data Completeness)

Unlike other policy types, lecture_performance policies require data preparation before the phase starts:
...
All threshold and achievement criteria live in `LecturePerformance::Rule` (regular columns, associations). Changing a threshold (e.g. 50 → 45) is done by updating the rule record via a diff modal, not the policy config. The JSONB usage rationale (generic policy kinds need flexible keyed configs) is documented centrally in the Registration chapter; this chapter only notes the minimal linkage.
...
```ruby
# Pseudo-code for Registration::Policy#evaluate(user) when kind == :lecture_performance
...
### Flowchart: Lecture Performance Policy Flow

```mermaid
flowchart TD
  Start([Teacher adds lecture_performance policy]) --> Phase{Which phase?}
...
2. After final homework deadline, a background job runs:
   ```ruby
   LecturePerformance::Service.new(lecture: linear_algebra).compute_and_upsert_all_records!
   ```
...
sequenceDiagram
  ...
  participant Cert as LecturePerformance::Certification
  ...
  Teacher->>Campaign: add lecture_performance policy (phase: registration)
...
- `lecture_performance_records` - Materialized per-user performance (facts only)
...
- `lecture_performance_certifications` - Teacher certification (pending/passed/failed) with audit and rule snapshot
