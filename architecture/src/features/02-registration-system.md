...
- Polymorphic Design: Any model can become registerable or host campaigns
...
## 1) RegistrationCampaign — The Registration Process Orchestrator
...
Key fields:
- host_type, host_id (polymorphic)
- title
- assignment_mode (enum): first_come_first_serve, preference_based
- status (enum): draft, open, processing, completed
- registration_deadline (datetime)
- Derived eligibility (new): Delegated to Policy Engine (policies may include prerequisite, exam eligibility, email domain, etc.)

Behavior highlights (updated):
...
Examples:
- Lecture hosts “Tutorial Registration” campaign for its tutorials
- Seminar hosts “Talk Assignment” campaign for its talks
- Exam hosts “Exam Registration” campaign with `exam_eligibility` and `institutional_email` policies.
...
```admonish info "What it represents"
A lightweight concern mixed into any domain model that can serve as the context or "host" for a `RegistrationCampaign`.
```
...
module Campaignable
  ...
  included do
    has_many :registration_campaigns, as: :host, dependent: :destroy
  end
end
```
...
Examples:
- Lecture hosts "Tutorial Registration" for students to pick tutorial sections
- Seminar hosts "Talk Assignment" for students to pick presentation topics
...
class RegistrationPolicy < ApplicationRecord
	...
	def eval_exam(user)
		lecture_id = config["lecture_id"] || registration_campaign.host_id
...
## 12) Campaign Lifecycle (State Diagram Placeholder)
...
erDiagram
	...
	REGISTRATION_CAMPAIGN }o--|| HOST : polymorphic
```
