...
- ViewComponent: `PointGridComponent` (since replaced by `ExamPointingTableComponent`, which draws every candidate on the roster with their status inline) — main table showed scoring participations with dynamic task columns and a total column; absent, exempt and not-submitted participations went to a separate "Special Cases" card.
...
- UI: `ExamPointingTableComponent` (points per task, save per row) and `ExamGradingTableComponent` (grade, absence and exemption per row through `AbsenceHandling`; the exemption's reason lives in its dialog) on the exam's dashboard. Tutorial-scoped view (same `index`, filtered): team-based table with per-task point inputs, progress indicator (graded / total), filter by graded/not graded, submission file links, auto-calculated totals.
