...
- UI: `ExamPointingTableComponent` (points per task, save per row) and `ExamGradingTableComponent` (grade, note, absence and exemption per row through `AbsenceHandling`) on the exam's dashboard. Tutorial-scoped view (same `index`, filtered): team-based table with per-task point inputs, progress indicator (graded / total), filter by graded/not graded, submission file links, auto-calculated totals.
