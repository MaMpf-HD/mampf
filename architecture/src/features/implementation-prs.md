...
- ViewComponent: `PointGridComponent` (since replaced by `ExamPointsTableComponent`, which draws every candidate on the roster with their status inline) — main table showed scoring participations with dynamic task columns and a total column; absent, exempt and not-submitted participations went to a separate "Special Cases" card.
- Rake: Seed the demo assessment data via `demo:assessment` / `Demo::SetupSupport.setup_assessment!` so `Assessment::TaskPoint` records and `participation.points_total` are populated for reviewed participations, while absent/exempt and not-submitted cases remain unscored and future-deadline assessments are skipped.
...
- UI: the grade is a select in the exam's grading rows (`ExamGradingTableComponent`), saved per row; absence and exemption are row actions, the exemption's reason is entered in a dialog and read on the status icon — there is no note field on the row
- Acceptance: Teachers can enter grades directly for any Gradable including talks; tutors can enter grades for exams when permitted by teacher; teachers can mark students as absent or exempt, one row at a time; an exemption carries its reason; validation works; audit tracking visible; feature flag gates UI.
```
...
- UI: `ExamPointsTableComponent` (points per task, save per row or all at once) and `ExamGradingTableComponent` (grade, absence and exemption per row through `AbsenceHandling`; the exemption's reason lives in its dialog) on the exam's dashboard. Tutorial-scoped view (same `index`, filtered): team-based table with per-task point inputs, progress indicator (graded / total), filter by graded/not graded, submission file links, auto-calculated totals.
...
- Dependencies: Requires PR-12.1 (model + participations exist). Can be developed in parallel with PRs 12.2/12.3/12.5.
...
- Note: This PR is intended for a different programmer. Empty/pending states are shown for not-yet-marked achievements.
