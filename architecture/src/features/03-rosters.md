...
| `in_campaign?`, `in_active_campaign?`, `in_completed_campaign?` | Provided | Helpers describing campaign association state. `in_campaign?` means "was ever in one" and gates the `skip_campaigns` switch; `in_active_campaign?` ignores finalized campaigns and is what deletion asks. |
...
| `destruction_blockers` | Provided (extendable) | The reasons that rule out destruction (`:in_campaign` — only while that campaign is still running —, `:roster_not_empty`, plus type-specific ones such as `:submissions` and `:media`). Models override it by appending to `super`. |
...
- **A finished process releases its groups:** `:in_campaign` asks
  `in_active_campaign?`, so a campaign that has been finalized no longer keeps a
  group alive. From then on the group's own rules decide, which is what the rest
  of MaMpf already assumed: `Rosters::NoCampaignRegisterablesQuery` lists such
  groups as manually managed, and `locked?` lets their rosters be edited again.
  `Registration::Registerable` therefore declares `dependent: :destroy` on its
  items — the association is polymorphic, so no foreign key would clear them.
